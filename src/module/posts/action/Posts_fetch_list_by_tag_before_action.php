...
final class Posts_fetch_list_by_tag_before_action {
  private PDO $pdo;
  ...
  public function __invoke(Request $request, Response $response, array $args): Response {
    ...
    $sql = 'SELECT pixelpost_pixelpost.id AS post_id, datetime, headline, 
      pixelpost_pixelpost.slug AS post_slug, body, 
      image, alt_headline, alt_body, comments, exif_info 
      FROM pixelpost_pixelpost INNER JOIN pixelpost_tags 
      ON pixelpost_tags.img_id = pixelpost_pixelpost.id 
      WHERE datetime < :datetime AND pixelpost_tags.tag = :slug 
      ORDER BY datetime DESC, pixelpost_pixelpost.id DESC LIMIT :limit';
    ...
    $links =['self' => "/posts/tag/{$slug}/before/{$page_datetime_decoded}?limit=$page_size"];
    ...
    if (sizeof($prev_page_post)> 0) {
      ...
      $links['previous'] = "/posts/tag/{$slug}/before/{$prev_datetime_uri}?limit=$page_size";
    }

    reset($posts_processed);
    $after_datetime_mysql = sizeof($posts_processed) > 0 ? current($posts_processed)['datetime'] : $page_datetime_mysql;

    $sql_next = 'SELECT pixelpost_pixelpost.id AS post_id, datetime, headline, 
      pixelpost_pixelpost.slug AS post_slug, body, 
      image, alt_headline, alt_body, comments, exif_info 
      FROM pixelpost_pixelpost INNER JOIN pixelpost_tags 
      ON pixelpost_tags.img_id = pixelpost_pixelpost.id 
      WHERE datetime > :datetime AND pixelpost_tags.tag = :slug 
      ORDER BY datetime ASC, pixelpost_pixelpost.id ASC LIMIT 1';
    ...
    if (sizeof($next_posts) > 0) {
      ...
      $links['next'] = "/posts/tag/{$slug}/after/{$next_datetime_uri}?limit=$page_size";
    } 
