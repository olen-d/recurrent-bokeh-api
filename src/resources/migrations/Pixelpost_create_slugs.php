...
namespace App\resources\migrations\convert;
// Access the database
...
final class Pixelpost_create_slugs {
  private PDO $pdo;
  ...
  public function __invoke(Request $request, Response $response): Response {
    ...
    $add_col_slug = $this->pdo->query(
      'ALTER TABLE pixelpost_pixelpost
      ADD slug CHAR(192) AFTER headline
      '
    );
  
    $add_idx_slug = $this->pdo->query(
      'CREATE INDEX idx_slug
      ON pixelpost_pixelpost (slug(16))'
    );
    ...
    foreach ($slugs_processed as $item) {
      ...
      $squirrel = 'UPDATE pixelpost_pixelpost SET slug=? WHERE id =?';
